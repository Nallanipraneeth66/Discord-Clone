...
const VoiceComponent = ({ item }) => {
  ...
  useEffect(() => {
    const handleActiveSpeakerChange = () => {
      ...
      setIsSpeaking(activeSpeakers.some((participant) => participant.identity === item.username));
    };
    room?.on("activeSpeakersChanged", handleActiveSpeakerChange);
    return () => { room?.off("activeSpeakersChanged", handleActiveSpeakerChange); };
  }, [room, item.username]);
    ...
    <div className="w-full flex flex-col space-y-2">
      <div className="flex items-center px-2 gap-4 rounded-lg transition-colors duration-300">
        <div className="flex items-center gap-1 md:gap-3 w-full justify-between">
          <img className={`w-5 h-5 md:w-7 md:h-7 rounded-full border-2 ${isSpeaking ? "border-green-500 border-2" : "border-blue-500"}`} src={`${url}${item.profilePic}`} alt={`${item.username}'s profile`} />
          <div className="flex w-full justify-between px-0 md:px-1">
            ...
            {isCurrentUser && (
              <div className="flex gap-1 md:gap-2 items-center text-xs md:text-sm">
                {!turnMicOff ? <FaMicrophone /> : <FaMicrophoneSlash className="text-red-600" />}
                {!turnHeadOff ? <FaHeadphones /> : <TbHeadphonesOff className="text-red-600" />}
              </div>
            )}
          </div>
        </div>
