...
const Home = ({isAreaOpen,setIsAreaOpen}) => {
  const {
    user,
    activeMenu,
    setActiveMenu,
  ...
  const [activeTopBarMenu, setActiveTopBarMenu] = useState("online");
  ...
  return (
    <div className={`w-full h-screen flex bg-bg2`}>
      ...
      {activeMenu === "friends" && (
        <div className="w-full md:w-[70%] flex flex-col px-10 md:px-0">
          ...
          {activeTopBarMenu === "online" && (
            <div className="w-full border-r border-bd0/70 h-[88%]">
              <div className="w-full h-12 flex  items-center  relative ">
                <div className="w-full h-[70%] relative flex justify-center">
                  <input
                    className="ui-input w-[95%] h-full"
                    placeholder="Search"
                    type="text"
                  />
                  <IoMdSearch className="absolute text-2xl text-fg2 right-6 bottom-1 mr-1" />
                </div>
              </div>

              <div className="w-full h-auto text-fg1 px-7 py-3 font-bold">
                ...
                {user?.friends && user?.friends.length > 0 ? (
                  onlineFriends.length > 0 ? (
                    ...
                    <div className="flex justify-center items-center h-full">
                      <div className="text-fg1 p-8 rounded-lg max-w-md text-center ui-panel">
                        <div className="flex flex-col items-center">
                          <div className=" p-4 rounded-full mb-4">
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              className="h-12 w-12 text-fg2"
                              fill="none"
                              viewBox="0 0 24 24"
                              stroke="currentColor"
                            >
                  ...
                  <div className="flex justify-center items-center h-full text-fg2 font-medium text-xl">
